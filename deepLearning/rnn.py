#rnn recurent neural network sp500
...
rnn = Sequential()
#1st hidden layer
rnn.add(Dense(num_neurons_in_hidden_layers, input_dim = num_lags, activation = 'relu'))
#2nd hidden layer
rnn.add(Dense(num_neurons_in_hidden_layers, activation = 'relu'))
# Output layer
rnn.add(Dense(1))
# Compiling
rnn.compile(loss = 'mean_squared_error', optimizer = 'adam')
# Fitting the model
rnn.fit(x_train, np.reshape(y_train, (-1, 1)), epochs = num_epochs,batch_size = batch_size)
# Predicting in-sample
y_predicted_train = np.reshape(rnn.predict(x_train), (-1, 1))
# Predicting out-of-sample
y_predicted = np.reshape(rnn.predict(x_test), (-1, 1))
